...
def set_num_threads(n): os.environ["OMP_NUM_THREADS"]=os.environ["MKL_NUM_THREADS"]=str(n); torch.set_num_threads(n)
...
class DualHeadAE(nn.Module):
    def __init__(self, fp_bits, md_dim, latent_dim):
        ...
        self.dec_fp  = nn.Sequential(nn.Linear(latent_dim, 512), nn.ReLU(),
                                     nn.Linear(512, fp_bits))   # logits for BCE
        self.dec_md  = nn.Sequential(nn.Linear(latent_dim, 512), nn.ReLU(),
                                     nn.Linear(512, md_dim))    # continuous for MSE/Huber
    def forward(self, x):
        z = self.encoder(x)
...
def train_from_memmaps(
    ...
    dl_val = DataLoader(torch.utils.data.TensorDataset(Xval), batch_size=batch_size, shuffle=False, num_workers=0)
    ...
    for ep in range(epochs):
        ...
        # csv
        pd.DataFrame({"epoch": np.arange(1,len(tr_hist)+1),"train_loss":tr_hist,"val_loss":va_hist})\
...
def evaluate_and_cluster(
    ...
    if eval_topk_features and eval_topk_features > 0 and mordred_dim > 0:
        ...
        for (xb,) in DataLoader(torch.utils.data.TensorDataset(Xval), batch_size=batch_size, shuffle=False):
            xb = xb.to(device)
            with torch.no_grad():
                fp_logits, md_out, _ = model(xb)
                probs = torch.sigmoid(fp_logits)
    ...
    if len(latents) >= 10:
        ...
        plt.title(f"Latent t-SNE clusters ({cluster_method})" + (f", silhouette={sil:.3f}" if sil is not None else ""))
...
# ------------------------- CLI -------------------------
def main():
    ap = argparse.ArgumentParser(description="FP + Mordred AE (dual-head) with evaluation, t-SNE & clustering")
    ...
    # compatibility flag (logged only)
    ...
    # NEW: bring back your old plot controls
    ...
    if args.cmd == "train":
        ...
        # Mordred names (for labeling top-K plots)
        ...
        if not os.path.exists(kept_stats):
            ...
            if not cand: raise FileNotFoundError("Could not find *_kept_stats.npz for Mordred memmap.")
            kept_stats = cand[0]
        ...
        meta = {"fp_bits": int(args.fp_bits), "latent_dim": int(args.latent_dim),
                "mordred_dim": int(md_dim), "fp_radius": (None if args.fp_radius is None else int(args.fp_radius))}
        with open(os.path.join(args.outdir, "run_meta.json"), "w") as f: json.dump(meta, f, indent=2)
        ...
        if not args.no_eval:
            evaluate_and_cluster(
                ...
                eval_topk_features=args.eval_topk_features, eval_bar_top=args.eval_bar_top, md_names=md_names
            )
